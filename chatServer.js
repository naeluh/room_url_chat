...
// ***************************************************************************
   ...
   io.sockets.on('connection', function(socket) {
    ...
    /* my implementation of io.socket.on ('connection')

io.sockets.on('connection', function(socket) {

    // Welcome message on connection
    socket.emit('connected', 'Welcome to the chat server');
    //socket.emit('room', prompt("What is the name of your room?"));
    ...
    socket.join(room);
    logger.emit('newEvent', 'userJoinsRoom', {'socket':socket.id, 'room':room});
    // Confirm subscription to user
    socket.emit('subscriptionConfirmed', {'room':room});
    // Notify subscription to all users in room
    var data = {'room':room, 'username':'anonymous', 'msg':'----- Joined the room -----', 'id':socket.id};
    io.sockets.in(room).emit('userJoinsRoom', data);
        ...
        socket.on('roomnames', function(data) {
         var rooms = _.clone(io.sockets.manager.roomClients[socket.id]);
            ...
            _.each(_.keys(rooms), function(room) {
                ...
                if (room) {
                    var message = {'room':room, 'username':obj.username, 'msg':'----- Left the room -----', 'id':obj.socketID};
                    io.sockets.in(room).emit('roomnames', message);
                }
            });
    });
