(function(){
    ...
    // Add Room in new window

    var addWindow = function(room){
        var url = '#'+room;
...
(function(){
    ...
    var socket = io.connect('http://hulea.org:8884');
    ...
    socket.on('subscriptionConfirmed', function(data) {
        ...
        if (!roomExists(data.room)) {
            var url = '#'+room;
            var win = window.open(url, '_blank');
            var addR1 = addRoomTab(data.room);
            var addR2 = addRoom(data.room);
            $(win.document.body).html(addR1,addR2);
            console.log(url);
            //addRoomTab(data.room);
            //addRoom(data.room);
    ...
    socket.on('unsubscriptionConfirmed', function(data) {
        ...
        if (roomExists(data.room)) {
            ...
            console.log('unsubscriptionConfirmed '+data.room);
    ...
    socket.on('newMessage', function (data) {
        ...
        // Scroll down room messages
        var room_messages = '#'+data.room+' #room_messages';
        console.log('newMessage '+data.room);
        $(room_messages).animate({
            scrollTop: $(room_messages).height()
        }, 300);
    });
    ...
    // Users in room received
    socket.on('usersInRoom', function(data) {
        console.log(getCurrentRoom(),getRoomName())
        console.log('usersInRoom: %s', JSON.stringify(data));
        _.each(data.users, function(user) {
            addUser(user);
        });
    ...
    socket.on('roomnames', function(data) {
        console.log('roomnames: %s', JSON.stringify(data));
        console.log('roomnames'+data.room);
        var r = $('input#room_name').val()
        console.log(r);
        addMessage(r);
    ...
    app_router.on('route:roomnames', function (room) {
        // Note the variable in the route definition being passed in here
        alert( "Get post number " + room );
    });
    ...
    /*$('#b_join_room').click(function (eventObject,room,data) {
      ...
      // Join new room
    $('#join_button').click(function (eventObject) {
        eventObject.preventDefault();
        socket.emit('subscribe', {'rooms':[getRoomName()] });
